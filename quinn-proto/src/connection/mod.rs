    ...
    fn handle_packet(
        ...
        if let Err(conn_err) = result {
            ...
            self.state = match conn_err {
                ...
                ConnectionError::TransmitFailed(_) => {
                    unreachable!("TransmitFailed isn't generated by packet processing");
                }
            };
        }
...
pub enum ConnectionError {
    ...
    CidsExhausted,
    /// The local network stack was unable to transmit to the peer
    ///
    /// Raised when a transmit fails with an error that retransmission cannot resolve, such as
    /// there being no route to the peer's address. Unlike [`TimedOut`], this is reported by the
    /// local platform as soon as it occurs, so an application can react promptly, e.g. by
    /// retrying the connection over a different address family.
    ///
    /// [`TimedOut`]: ConnectionError::TimedOut
    #[error("transmit failed: {0}")]
    TransmitFailed(io::ErrorKind),
}
